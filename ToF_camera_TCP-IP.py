...
# En este caso utilizamos la libreria socket para TCP
...
if __name__ == "__main__":
    ...
    # Indicamos el HOST y el puerto, y creamos el socket
    ...
    # aca podemos ver si hay un error en el socket creado
    ...
    (conn, addr) = s.accept()
    print(f"Conexion de {addr} ha sido establecida")

    # Initialize OpenNI
    if platform.system() == "Windows":
        # Specify path for Redist
    ...
    while cv2.waitKey(1) == -1 and cv2.getWindowProperty("Depth View", cv2.WND_PROP_FULLSCREEN) != -1:
        ...
        # Leemos aca la data recibida

        data = conn.recv(1024)

        # la decodificamos a utf-8

        full_data += data.decode('utf-8')
        #print('Send message back in response to: '+ data)
        ...
        for i in range(0, len(list_archivos)):
            ...
            if (full_data + ".txt") == list_archivos[i]:
                ...
                with open(full_data + ".txt", "r") as myfile:
                ...
                # los mensajes los mandamos a traves de ese reply
        ...
        if full_data == 'conteo':
            ...
            # enviamos a traves de reply

            reply = conteo_json

        elif full_data == 'reboot':


            reply = 'Reiniciar dispositivo'

        elif full_data == 'status':
            ...
            reply = 'Comando invalido. Comandos disponibles "conteo", "status","log", "mode","newData"'
        # enviamos el reply
        full_data = ''
        conn.send(bytes(reply, "utf-8"))
        
    
        # ahora leemos el frame
        ...
        rgb_frame = cv2.applyColorMap(
            depth_array_norm.astype(np.uint8), cv2.COLORMAP_WINTER)
        ...
        if (none_background):
            ...
            time_data = dia_semana+","+ \
                str(año)+"/"+str(mes_num)+"/"+str(dia)+","+str(hora)+":"+str(minuto)+":"+str(segundo)
        ...
        if nro_personas_entrada > 1:
            ...
            with open(file_month, "a") as myfile:
                myfile.write("Ingresos,"+time_data+"," + \
                             str(contador_final_entrada)+"\n")
        ...
        if nro_personas_salida > 1:
            ...
            with open(file_month, "a") as myfile:
                myfile.write("Salidas,"+time_data+"," + \
                             str(contador_final_salida)+"\n")
